...
export default function EditCategoryPage({ params }: { params: { id: string } }) {
  ...
  const fetchCategories = async () => {
    try {
      ...
      // Mevcut kategoriyi ve alt kategorilerini listeden çıkar
      setCategories(data.filter(category => 
        category.id !== params.id && 
        !isSubCategory(category, params.id)
      ));
    } catch (error) {
  ...
  // Bir kategorinin alt kategori olup olmadığını kontrol et
  const isSubCategory = (category: KnowledgeBaseCategory, parentId: string): boolean => {
    if (category.parentId === parentId) return true;
    const parent = categories.find(c => c.id === category.parentId);
    if (!parent) return false;
    return isSubCategory(parent, parentId);
  };
